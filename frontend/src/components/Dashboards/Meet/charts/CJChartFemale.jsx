...
function CJChartFemale () {
    const {data: chart } = useSelector((state)=>state.meet)

    const makes = chart.womens.chart_data.attempt_history_chart["C&J"].Make.map((subArray) => ({
        x: subArray[1], 
        y: subArray[2],
        label: subArray[0] 
    }));

    const misses = chart.womens.chart_data.attempt_history_chart["C&J"].Miss.map((subArray) => ({
        x: subArray[1], 
        y: subArray[2], 
        label: subArray[0] 
    }));
